...
class IndexingService {
  ...
  async findProductByUrl(url) {
    try {
      console.log('[findProductByUrl] Searching for URL:', url);
      ...
      console.log('[findProductByUrl] Search results:', response.body.hits.total);

      if (response.body.hits.hits.length > 0) {
        const hit = response.body.hits.hits[0];
        console.log('[findProductByUrl] Found product:', hit._id, hit._source.name);
        return {
          id: hit._id,
          ...hit._source
        };
      ...
      console.log('[findProductByUrl] No product found for URL:', url);
      return null;
    } catch (error) {
      console.error('[findProductByUrl] Error:', error);
      throw error;
    }
  }

  /**
   * Scrape fresh data from a website
  ...
  async scrapeWebsite(url) {
    try {
      ...
      return {
        ...
        twitter_creator: pageContent.twitter_creator,
        twitter_site: pageContent.twitter_site
      };
    } catch (error) {
